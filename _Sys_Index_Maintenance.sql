...
(
  ...
  @FragmentationLowLevel int = 5,
  @FragmentationHighLevel int = 30,
...
BEGIN
  ...
  DECLARE @StartMessage nvarchar(max),
      ...
      @CurrentAvailabilityGroup nvarchar(max),
      @CurrentAvailabilityGroupRole nvarchar(max),
      @CurrentDatabaseMirroringRole nvarchar(max),
      @CurrentIsReadOnly bit,
      @CurrentCommand01 nvarchar(max),
      @CurrentCommand02 nvarchar(max),
      @CurrentCommand03 nvarchar(max),
      @CurrentCommand04 nvarchar(max),
      @CurrentCommand05 nvarchar(max),
      @CurrentCommand06 nvarchar(max),
      @CurrentCommand07 nvarchar(max),
      @CurrentCommand08 nvarchar(max),
      @CurrentCommand09 nvarchar(max),
      @CurrentCommand10 nvarchar(max),
      @CurrentCommand11 nvarchar(max),
      @CurrentCommand12 nvarchar(max),
      @CurrentCommand13 nvarchar(max),
      @CurrentCommand14 nvarchar(max),
      @CurrentCommandOutput13 int,
      @CurrentCommandOutput14 int,
      @CurrentCommandType13 nvarchar(max),
      @CurrentCommandType14 nvarchar(max),
      @CurrentIxID int,
      @CurrentSchemaID int,
  ...
  SET @StartMessage = @StartMessage + ', @FragmentationLowLevel = ' + ISNULL(CAST(@FragmentationLowLevel AS nvarchar),'NULL')
  SET @StartMessage = @StartMessage + ', @FragmentationHighLevel = ' + ISNULL(CAST(@FragmentationHighLevel AS nvarchar),'NULL')
  ...
  IF @FragmentationLowLevel <= 0 OR @FragmentationLowLevel >= 100 OR @FragmentationLowLevel >= @FragmentationHighLevel OR @FragmentationLowLevel IS NULL
  BEGIN
    SET @ErrorMessage = 'The value for the parameter @FragmentationLowLevel is not supported.' + CHAR(13) + CHAR(10) + ' '
    RAISERROR(@ErrorMessage,16,1) WITH NOWAIT
    SET @Error = @@ERROR
  END

  IF @FragmentationHighLevel <= 0 OR @FragmentationHighLevel >= 100 OR @FragmentationHighLevel <= @FragmentationLowLevel OR @FragmentationHighLevel IS NULL
  BEGIN
    SET @ErrorMessage = 'The value for the parameter @FragmentationHighLevel is not supported.' + CHAR(13) + CHAR(10) + ' '
    RAISERROR(@ErrorMessage,16,1) WITH NOWAIT
    SET @Error = @@ERROR
  END
  ...
  IF @Error <> 0
  BEGIN
    SET @ErrorMessage = 'The process errored out.' + CHAR(13) + CHAR(10) + ' '
    RAISERROR(@ErrorMessage,16,1) WITH NOWAIT
    SET @ReturnCode = @Error
    GOTO Logging
  END
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          WHEN @CurrentFragmentationLevel >= @FragmentationHighLevel THEN 'High'
          WHEN @CurrentFragmentationLevel >= @FragmentationLowLevel AND @CurrentFragmentationLevel < @FragmentationHighLevel THEN 'Medium'
          WHEN @CurrentFragmentationLevel < @FragmentationLowLevel THEN 'Low'
          END
        END
